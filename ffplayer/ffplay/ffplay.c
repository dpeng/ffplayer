...
void ffplay_stop(void)
{
	...
	{/*
		if((m_streamIndex[AVMEDIA_TYPE_VIDEO] >= 0) && (NULL != m_curstream->ic))
			avcodec_close(m_curstream->ic->streams[m_streamIndex[AVMEDIA_TYPE_VIDEO]]);
		if((m_streamIndex[AVMEDIA_TYPE_AUDIO] >= 0) && (NULL != m_curstream->ic))
			avcodec_close(m_curstream->ic->streams[m_streamIndex[AVMEDIA_TYPE_AUDIO]]);*/
		do_exit(m_curstream);
		m_curstream = NULL;
	}
}
